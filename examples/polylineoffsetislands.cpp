...
int main(int argc, char *argv[]) {
  ...
  cavccpp::Polyline<double> outerCCWLoop;
  outerCCWLoop.addVertex(-10, 1, 1);
  outerCCWLoop.addVertex(10, 1, 1);
  outerCCWLoop.isClosed() = true;

  cavccpp::Polyline<double> island1;
  ...
  cavccpp::invertDirection(island1);

  cavccpp::Polyline<double> island2;
  ...
  cavccpp::OffsetLoopSet<double> loopSet;
  ...
  loopSet.ccwLoops.push_back({0, outerCCWLoop, cavccpp::createApproxSpatialIndex(outerCCWLoop)});

  // add the clockwise loop islands
  loopSet.cwLoops.push_back({0, island1, cavccpp::createApproxSpatialIndex(island1)});
  loopSet.cwLoops.push_back({0, island2, cavccpp::createApproxSpatialIndex(island2)});
  ...
  cavccpp::ParallelOffsetIslands<double> alg;
  ...
  cavccpp::OffsetLoopSet<double> offsetResult = alg.compute(loopSet, offsetDelta);
  // compute offset of first offset result (offset by another 1.0)
  cavccpp::OffsetLoopSet<double> offsetResult2 = alg.compute(offsetResult, offsetDelta);
}
