...
inline cavc_pline *plineFromVertexes(std::vector<cavc_vertex> const &vertexes, bool isClosed) {
  cavc_vertex const *vertex_data = vertexes.empty() ? nullptr : &vertexes[0];
  return cavc_pline_new(vertex_data, static_cast<uint32_t>(vertexes.size()), isClosed ? 1 : 0);
}

// reverses the direction of the polyline defined by vertexes
inline void reverseDirection(std::vector<cavc_vertex> &vertexes) {
  if (vertexes.size() < 2) {
    return;
  }
...
inline cavc_pline *createRevseredPline(cavc_pline *pline) {
...
inline cavc_parallel_offset_options defaultParallelOffsetOptions() {
  return cavc_parallel_offset_default_options();
}
